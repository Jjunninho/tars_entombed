...
//   TRAINING  — treina a rede em background (headless acelerado)
//   PLAYING   — IA controla o TARS em tempo real
// ============================================================
...
window.AIController = (function () {
    ...
    // ── Estado Interno ─────────────────────────────────────────────
    const MODES = { HUMAN: 'HUMAN', RECORDING: 'RECORDING', TRAINING: 'TRAINING', PLAYING: 'PLAYING' };
    ...
    function rlStep() {
        ...
        // Avança um tick do jogo (sem render)
        let done = false;
        if (GAME.health <= 0 || player.y > GAME.cameraY + 800 + 300) {
            done = true;
        } else {
            update(); // chama a função update() do main.js
        }
    ...
    function updateHUD() {
        ...
        const modeColor = {
            HUMAN: '#aaa', RECORDING: '#f44', TRAINING: '#ff0', PLAYING: '#0f0'
        }[currentMode] || '#fff';
    ...
    // ── Hook no gameLoop original ──────────────────────────────────
    // Intercepta cada frame para injetar recorder tick e ai play tick
    const _origGameLoop = window.gameLoop;
    window.gameLoop = function () {
        update();

        // Modo RECORDING: captura estado+ação após update
        if (currentMode === MODES.RECORDING) {
            AIRecorder.tick();
        }

        // Modo PLAYING: injeta ação da IA para o próximo frame
        if (currentMode === MODES.PLAYING) {
            aiPlayTick();
        }
        ...
        // Atualiza HUD a cada 60 frames
        if (GAME.frameCount % 60 === 0) updateHUD();

        if (!rafStopped) _origRAF(window.gameLoop);
    };
    ...
    return {
        ...
        init() {
            ...
            // Tenta carregar cérebro existente
...
document.addEventListener('DOMContentLoaded', () => {
    // Pequeno delay para garantir que main.js inicializou o gameLoop
    setTimeout(() => {
        AIController.init();
