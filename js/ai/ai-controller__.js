...
window.AIController = (function () {
    ...
    // ── Hook no update() ──────────────────────────────────────────
    // update() é chamado a cada frame pelo gameLoop nativo.
    // Hookar aqui é seguro: não interfere no requestAnimationFrame.
    function installUpdateHook() {
        ...
        _headlessUpdate = _origUpdate; // salva para uso no headless RL

        window.update = function () {
            _origUpdate();

            // Modo RECORDING: captura estado+ação logo após update
            if (currentMode === MODES.RECORDING) {
                AIRecorder.tick();
            }
